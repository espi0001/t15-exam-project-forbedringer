...
const InfiniteBanner = ({ items = [], speed = "slow" }) => {
  ...
  const repeatCount = 10; // Reduce to just duplicating once
  ...
  return (
    <div className="relative w-full overflow-hidden mt-10">
      <div className="w-full inline-flex will-change-transform animate-infinite-scroll font-thin">
        {keyedItems.map(({ content, key }) => (
          <div key={key} className="flex items-center space-x-4 px-4 shrink-0 text-red_color">
            {content}
          </div>
        ))}
      </div>
