...
const Hero = () => {
  ...
  useEffect(() => {
    // Tjek, om brugeren har besøgt siden før denne session
    const hasVisited = sessionStorage.getItem("hasVisited");

    if (!hasVisited) {
      // Første besøg: Gem i sessionStorage og start animationen
      ...
      tl.to([".text-layer-1 p", ".text-layer-1 h1", ".text-layer-2 p", ".text-layer-2 h1"], {
        ...
        .call(() => {
          // Fjern overlay efter animationen er færdig
          setShowOverlay(false);
        });
    } else {
      // Hvis brugeren allerede har besøgt siden, fjernes overlayet
      setShowOverlay(false);
    }
  }, []);
  ...
  return (
    <div className="relative w-full h-screen">
      {/* Loading overlay (kun vises første gang) */}
      {showOverlay && (
        <div
